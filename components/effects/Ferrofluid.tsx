...
function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (mouseDampening <= 0) uniforms.iMouse.value = [mouseTarget[0], mouseTarget[1]];
    };
    ...
    colors, speed, scale, turbulence, fluidity, rimWidth, sharpness, shimmer, glow,
    flowDirection, opacity, mouseInteraction, mouseStrength, mouseRadius, mouseDampening,
  ]);

  return <div ref={containerRef} className={`ferrofluid-container ${className ?? ""}`} />;
}

export default memo(Ferrofluid);
